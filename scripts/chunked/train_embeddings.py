...
for i in range(1, corpus_file_num + 1):
	with open(os.path.dirname(__file__) + "/../../datasets/" + directory_name + "/Byte_Pairs/" + corpus_name + "_" + str(i) + "-byte_pairs.json", "r", encoding = 'utf-8') as f:
		print('\r' + str(i) + " / " + str(corpus_file_num), end = '')
		temp_byte_pairs = json.load(f)
		for s in temp_byte_pairs:
			byte_pairs.append(s)
print()
